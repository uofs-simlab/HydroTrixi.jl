function animate_solution_1d(sol;
    ...
    minimum(indices) >= firstindex(sol.t) || throw(ArgumentError("Frame index out of bounds."))
    maximum(indices) <= lastindex(sol.t) || throw(ArgumentError("Frame index out of bounds."))
    ...
    if !isnothing(exact_solution)
        ...
        y_exact_obs = Observable([_scalar_value(exact_solution(Trixi.SVector(xi), sol.t[first_idx]))
                                  for xi in x_exact])
        lines!(ax, x_exact, y_exact_obs;
               label=exact_label,
               linewidth=linewidth,
               linestyle=:dash,
