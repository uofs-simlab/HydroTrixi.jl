...
saturation following Ireson et al. (2023). For ``\psi < 0`` the effective
saturation is
```math
...
``\theta(\psi) = \theta_r + (\theta_s - \theta_r) S_e(\psi)`` follows from the
generic [`water_content`](@ref) relation using the residual and saturated water
