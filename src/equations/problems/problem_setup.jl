...
A container for reusable hydrologic PDE problem data. It stores the governing
`equations`, an `initial_condition`, `boundary_conditions`, the spatial
`domain`, the time interval `tspan`, and an optional `constitutive_relation`
for mixed-form problems such as Richards' equation.

The `domain` must be a pair `(x_min, x_max)` of coordinate tuples with matching
dimension. The type parameter `NDIMS` records that dimension for dispatch and
introspection.
"""
...
function SemidiscretizationConstitutive(mesh, hydrologic_problem::HydrologicProblem, solver;
                                        solver_parabolic,
                                        kwargs...)
    ...
    semi = Trixi.SemidiscretizationParabolic(mesh,
                                             ...
                                             kwargs...)
    return SemidiscretizationConstitutive(semi, constitutive_relation)
end
