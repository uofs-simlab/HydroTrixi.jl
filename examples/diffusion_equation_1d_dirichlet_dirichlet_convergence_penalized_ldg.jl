...
function boundary_conditions_dirichlet(h)
    ...
    return (; x_neg=BoundaryConditionDirichlet(zero_dirichlet),
...
function boundary_conditions_penalized_dirichlet(h)
    ...
    return (; x_neg=BoundaryConditionDirichletPenalty(zero_dirichlet; penalty=tau),
            x_pos=BoundaryConditionDirichletPenalty(zero_dirichlet; penalty=tau))
end

schemes = (
    (; name="LDG", boundary_builder=boundary_conditions_dirichlet),
    (; name="LDG + boundary penalty", boundary_builder=boundary_conditions_penalized_dirichlet),
)
...
function run_level(level, scheme)
    ...
    dt = dt_factor * h^2
    sol = SciMLBase.solve(ode, default_algorithm();
                          dt=dt,
                          adaptive=false,
