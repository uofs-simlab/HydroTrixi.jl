...
trixi_include(joinpath(@__DIR__, "elixir_richards_celia_1990.jl"); saveat = 0.0:2.0:360.0)
...
output_path = joinpath(plots_dir, "richards_celia_1990_pressure_head.$(animation_format)")
mass_bias_output_path = joinpath(plots_dir, "richards_celia_1990_mass_bias.$(plot_format)")

animate_solution_1d(sol; component = 2, xlabel = L"$z$ (m)", ylabel = L"$\psi$ (m)",
                    ylims = (-0.65, -0.15), output_path = output_path, framerate = 30)

plot_mass_bias(sol; output_path = mass_bias_output_path, xlabel = L"$t$ (s)",
               ylabel = L"$\epsilon_b$ (m)", xticks = 0.0:60.0:360.0)

println("Saved Celia ψ(z,t) animation to: $(output_path)")
println("Saved Celia mass-bias plot to: $(mass_bias_output_path)")
