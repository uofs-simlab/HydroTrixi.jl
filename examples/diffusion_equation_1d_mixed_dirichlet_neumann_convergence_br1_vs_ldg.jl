...
schemes = (
    (; name="LDG", parabolic_scheme=ViscousFormulationLocalDG()),
    (; name="BR1", parabolic_scheme=ViscousFormulationBassiRebay1()),
)
...
function run_level(level, scheme)
    case = mixed_dirichlet_neumann_case(; diffusivity=diffusivity,
                                        ...
                                        tspan=tspan,
                                        parabolic_scheme=scheme.parabolic_scheme,
...
function run_scheme(scheme)
    ...
    return (; ndofs, l2_errors, linf_errors, dts,
...
for scheme in schemes
    ...
    for i in eachindex(levels)
        @printf("%-7d %-10d %-11.3e %-14.6e ",
                levels[i], data.ndofs[i], data.dts[i], data.l2_errors[i])
...
scatterlines!(ax, ldg.ndofs, ldg.l2_errors;
              label=L"\mathrm{LDG}\ L^2",
              color=colors[1],
              linestyle=:solid,
              linewidth=1.8)
scatterlines!(ax, br1.ndofs, br1.l2_errors;
              label=L"\mathrm{BR1}\ L^2",
              color=colors[2],
              linestyle=:solid,
              linewidth=1.8)
scatterlines!(ax, ldg.ndofs, ldg.linf_errors;
              label=L"\mathrm{LDG}\ L^\infty",
              color=colors[1],
              linestyle=:dash,
              linewidth=1.8)
scatterlines!(ax, br1.ndofs, br1.linf_errors;
