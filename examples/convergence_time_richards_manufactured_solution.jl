...
function compute_time_eoc(errors, time_steps)
    if length(errors) != length(time_steps)
    ...
    for i in (firstindex(errors) + 1):lastindex(errors)
        values[i] = log(errors[i - 1] / errors[i]) / log(time_steps[i - 1] / time_steps[i])
    end

    return values
end

function run_time_step(elixir, scheme, time_step; final_time, initial_refinement_level,
                       polydeg, reltol, abstol, saveat, show_run_output)
    run_include = () -> Trixi.trixi_include(@__MODULE__, elixir; final_time = final_time,
                                            initial_refinement_level =
                                            initial_refinement_level, polydeg = polydeg,
                                            form = scheme.form, dt = time_step,
                                            adaptive = false, reltol = reltol,
                                            abstol = abstol, saveat = saveat,
                                            run_simulation = true)
...
function run_richards_temporal_study(elixir, schemes, time_steps; final_time,
                                     initial_refinement_level, polydeg, reltol, abstol,
                                     saveat, show_run_output)
    ...
    for scheme in schemes
        ...
        for time_step in time_steps
            ...
            run_data = run_time_step(elixir, scheme, time_step; final_time = final_time,
                                     initial_refinement_level = initial_refinement_level,
                                     polydeg = polydeg, reltol = reltol, abstol = abstol,
                                     saveat = saveat, show_run_output = show_run_output)
    ...
    open(output_path, "w") do io
        ...
        for scheme in schemes
            ...
            for i in eachindex(data.time_steps)
                ...
                @printf(io, "%s %.17e %d %.17e %.17e %.17e %s %s %s\n", scheme.name,
...
function read_temporal_table(input_path, schemes)
    results = Dict{String, Any}()
    for scheme in schemes
        results[scheme.name] = (; time_steps = Float64[], final_times = Float64[],
                                nsteps = Int[], l2_errors = Float64[],
                                linf_errors = Float64[], retcodes = String[])
    end
...
function plot_positive_order_triangle!(ax, x_left, x_right, y_ref, order;
    ...
    if x_right <= x_left
        throw(ArgumentError("`x_right` must exceed `x_left`."))
    end
    ratio = x_right / x_left
    y_left = y_ref / triangle_shift
    y_right = y_left * ratio^order

    lines!(ax, [x_left, x_right, x_right, x_left], [y_left, y_left, y_right, y_left];
           color = :black)
...
function plot_richards_temporal_study(schemes, results, output_path; triangle_order = 5,
                                      triangle_side = "right", triangle_shift = 1.5,
                                      ylims = nothing, output_png_path = nothing)
    ...
    time_step_ticks = Float64.(sort(unique(all_time_steps)))
    ...
    ax = Axis(fig[1, 1]; xlabel = L"$\Delta t$ (s)",
    ...
    for scheme in schemes
        ...
        scatterlines!(ax, data.time_steps[order], data.l2_errors[order];
                      label = scheme.label_l2, color = colors[scheme.color],
                      linestyle = :solid, linewidth = 1.8, marker = scheme.marker,
                      markersize = scheme.markersize)
        scatterlines!(ax, data.time_steps[order], data.linf_errors[order];
                      label = scheme.label_linf, color = colors[scheme.color],
                      linestyle = :dash, linewidth = 1.8, marker = scheme.marker,
                      markersize = scheme.markersize)
    end

    if minimum(length(results[scheme.name].time_steps) for scheme in schemes) < 2
        throw(ArgumentError("At least two time steps are required."))
    end

    sorted_time_steps = sort(unique(all_time_steps))
    triangle_indices = (; left = (1, 2),
                        right = (length(sorted_time_steps) - 1,
                                 length(sorted_time_steps)))
    left_index, right_index = getproperty(triangle_indices, Symbol(triangle_side))
    x_left = sorted_time_steps[left_index]
    x_right = sorted_time_steps[right_index]
    y_ref = Inf
    for scheme in schemes
        data = results[scheme.name]
    ...
    plot_positive_order_triangle!(ax, x_left, x_right, y_ref, triangle_order;
                                  triangle_shift = triangle_shift, trianglefontsize = 15,
                                  font = plot_font)
...
if plot_results
    convergence_plot = joinpath(plots_dir, "$(result_prefix).$(plot_format)")
    ...
    plot_richards_temporal_study(schemes, results, convergence_plot;
                                 ...
                                 triangle_shift = triangle_shift, ylims = plot_ylims,
