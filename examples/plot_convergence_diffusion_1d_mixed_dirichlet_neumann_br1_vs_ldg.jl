...
ax = Axis(fig[1, 1]; xlabel = LaTeXString("Degrees of freedom"),
          ylabel = LaTeXString("Error"), xlabelfont = plot_font, ylabelfont = plot_font,
          xticklabelfont = plot_font, yticklabelfont = plot_font, xscale = log10,
...
scatterlines!(ax, ldg.ndofs, ldg.l2_errors; label = L"\mathrm{LDG}\ L^2", color = colors[1],
              linestyle = :solid, linewidth = 1.8, markersize = 7.0)
scatterlines!(ax, br1.ndofs, br1.l2_errors; label = L"\mathrm{BR1}\ L^2", color = colors[2],
              linestyle = :solid, linewidth = 1.8, markersize = 7.0)
scatterlines!(ax, ldg.ndofs, ldg.linf_errors; label = L"\mathrm{LDG}\ L^\infty",
              color = colors[1], linestyle = :dash, linewidth = 1.8, markersize = 7.0)
scatterlines!(ax, br1.ndofs, br1.linf_errors; label = L"\mathrm{BR1}\ L^\infty",
              color = colors[2], linestyle = :dash, linewidth = 1.8, markersize = 7.0)

triangle_order = 4
x_ref_left = ldg.ndofs[end - 1]
x_ref_right = ldg.ndofs[end]
y_ref = min(ldg.l2_errors[end - 1], ldg.linf_errors[end - 1], br1.l2_errors[end - 1],
            br1.linf_errors[end - 1])
HydroTrixi.plot_bottom_triangle!(ax, x_ref_left, x_ref_right, y_ref, triangle_order;
