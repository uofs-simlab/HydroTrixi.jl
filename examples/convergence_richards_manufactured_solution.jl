...
function solve_refinement_level(elixir, scheme, level;
                                final_time,
                                polydeg,
                                reltol,
                                abstol,
...
function run_richards_refinement_study(elixir, schemes;
...
function read_refinement_table(input_path, schemes)
    ...
    results = Dict{String, Any}()
    for scheme in schemes
        results[scheme.name] = (; levels = Int[],
                                ndofs = Int[],
                                final_times = Float64[],
                                l2_errors = Float64[],
                                linf_errors = Float64[])
...
function plot_richards_refinement_study(schemes, results, output_path;
                                        polydeg,
    ...
    isempty(all_ndofs) && throw(ArgumentError("No refinement data found."))
    ...
    minimum(length(results[scheme.name].ndofs) for scheme in schemes) >= 2 ||
    ...
    HydroTrixi.plot_bottom_triangle!(ax,
                                     reference_data.ndofs[end - 1],
                                     reference_data.ndofs[end],
                                     y_ref,
                                     polydeg + 1;
                                     triangle_shift = 1.5,
                                     trianglefontsize = 15,
...
if plot_results
    convergence_plot = joinpath(plots_dir,
                                "$(result_prefix)_convergence.$(plot_format)")
    convergence_plot_png = joinpath(plots_dir, "$(result_prefix)_convergence.png")
    plot_richards_refinement_study(schemes, results, convergence_plot;
                                   polydeg = polydeg,
